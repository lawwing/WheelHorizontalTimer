...
public class WheelHorTimerView extends View {
    ...
    public WheelHorTimerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void initViewParam(float defaultValue, float minValue, float maxValue, int spanValue) {
        ...
        this.mTotalLine = (int) (maxValue * spanNum - minValue * spanNum) / spanValue + 1;
        mMaxOffset = -(mTotalLine - 1) * mItemSpacing;

        mOffset = (minValue - defaultValue) / spanValue * mItemSpacing * spanNum;
        invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTotalLine; i++) {
            ...
            if (i % spanNum == 0) { // 大指标,要标注文字
                value = String.valueOf((int) (mMinValue + i * mPerSpanValue / spanNum));
                //刻度文字的两边透明度
    ...
    private void countMoveEnd() {
        ...
        mValue = mMinValue + Math.round(Math.abs(mOffset) * 1.0f / mItemSpacing) * mPerSpanValue / spanNumf;
        mOffset = (mMinValue - mValue) * spanNumf / mPerSpanValue * mItemSpacing; // 矫正位置,保证不会停留在两个相邻刻度之间
        notifyValueChange();
        postInvalidate();
    }
